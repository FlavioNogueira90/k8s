...
resource "azurerm_kubernetes_cluster" "aks-aula-infra" {
  ...
  default_node_pool {
    name       = "default"
    node_count = 2                    # qtd de maquinas no cluster
    vm_size    = "Standard_D2_v2"     # tipo da maquina
  }
...
# Permissao para que o aks possa puxar imagem do registry
